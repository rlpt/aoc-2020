...
part1 =
    let
        ...
        okItems1 =
            items |> List.filter checkItem1
    in
    List.length okItems1
...
part2 =
    let
        ...
        okItems2 =
            items |> List.filter checkItem2
    in
    List.length okItems2
...
checkItem2 : Item -> Bool
checkItem2 item =
    let
        pos1 =
            checkCharAt (item.min - 1) item.char item.password

        pos2 =
            checkCharAt (item.max - 1) item.char item.password
    in
    [ pos1, pos2 ] |> List.filter identity |> List.length |> (==) 1


checkCharAt : Int -> Char -> String -> Bool
checkCharAt position char str =
    case getCharAt position str of
        Just c ->
            c == char
...
getCharAt position str =
    str |> String.toList |> List.drop position |> List.head


checkItem1 : Item -> Bool
checkItem1 item =
    let
        numOfMatchingChars =
            item.password |> String.toList |> List.filter ((==) item.char) |> List.length
    in
    if numOfMatchingChars >= item.min && numOfMatchingChars <= item.max then
        True
...
parse : String -> List Item
parse str =
    -- YOLO dont check if parsing failed
    str |> String.lines |> List.map parseLine |> List.filterMap identity


parseLine : String -> Maybe Item
parseLine str =
    case String.split " " str of
        [ rawRange, rawChar, rawPassword ] ->
            parseParts rawRange rawChar rawPassword

        _ ->
            Nothing


parseParts : String -> String -> String -> Maybe Item
parseParts rawRange rawChar password =
    let
        maybeRange =
            case String.split "-" rawRange of
                [ minStr, maxStr ] ->
                    ( String.toInt minStr, String.toInt maxStr )

                _ ->
                    ( Nothing, Nothing )

        maybeChar =
            case String.uncons rawChar of
                Just ( char, _ ) ->
                    Just char
                ...
                Nothing ->
                    Nothing
    in
    case ( Tuple.first maybeRange, Tuple.second maybeRange, maybeChar ) of
        ( Just min, Just max, Just char ) ->
            Just <| Item min max char password
