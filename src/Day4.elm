...
part1 =
    let
        validPassports =
            passports |> List.filter isValidPassport

        validNorthPole =
            passports |> List.filter isValidNorthPole
    in
    List.length validPassports + List.length validNorthPole


part2 : Int
part2 =
    List.filter allFieldsValid passports |> List.length


passports : List (Dict String String)
passports =
    chunk [] [] (String.lines data |> List.map String.trim)
        |> List.map toPairs
        |> List.map Dict.fromList


allFieldsValid : Dict String String -> Bool
allFieldsValid passport =
    let
        tests =
            [ isValidNumber 1920 2002 (getValue passport byr)
            , isValidNumber 2010 2020 (getValue passport iyr)
            , isValidNumber 2020 2030 (getValue passport eyr)
            , isValidHeight (getValue passport hgt)
            , isValidEyeColor (getValue passport ecl)
            , isValidHex (getValue passport hcl)
            , isValidPassportId (getValue passport pid)
            ]

        passed =
            tests |> List.filter identity
    in
    List.length tests == List.length passed


getValue : Dict String String -> String -> String
getValue dict key =
    Dict.get key dict |> Maybe.withDefault ""


isValidPassport : Dict String String -> Bool
isValidPassport passport =
    let
        reqPassport =
            [ byr, iyr, eyr, hgt, hcl, ecl, pid, cid ]
    in
    checkItems reqPassport passport


isValidNorthPole : Dict String String -> Bool
isValidNorthPole passport =
    let
        reqNorthPole =
            [ byr, iyr, eyr, hgt, hcl, ecl, pid ]

        hasRequiredFields =
            checkItems reqNorthPole passport

        hasCid =
            Dict.member cid passport
    in
    hasRequiredFields && not hasCid
...
isValidPassportId str =
    let
        ...
        validCharsLength =
            chars |> List.filter isValidDigit |> List.length
    in
    length == 9 && validCharsLength == 9
...
isValidHex str =
    let
        ...
        symbol =
            List.head chars |> Maybe.withDefault ' '

        value =
            List.tail chars |> Maybe.withDefault []

        allowed =
            "abcdef0123456789" |> String.toList

        valueLength =
            List.length value

        validCharsLength =
            value |> List.filter (\v -> List.member v allowed) |> List.length
    in
    symbol == '#' && valueLength == 6 && validCharsLength == 6
...
isValidHeight str =
    ...
    case unit of
        "cm" ->
            isValidNumber 150 193 numStr

        "in" ->
            isValidNumber 59 76 numStr
...
isValidNumber : Int -> Int -> String -> Bool
isValidNumber min max str =
...
checkItems : List String -> Dict String String -> Bool
checkItems toCheck passport =
    toCheck
        |> List.map
            (\k ->
                Dict.get k passport
...
{-| turns ["cid:252","pid:543685203 hcl:#c0946f"] into [("cid","252"), ("pid","543685203"), ("hcl","#c0946f"), ]
-}
toPairs : List String -> List ( String, String )
toPairs list =
    let
        parts =
            list |> List.map (String.split " ") |> List.concat
...
chunk : List String -> List (List String) -> List String -> List (List String)
chunk group allGroups input =
    case List.head input of
        Just head ->
            let
                tail =
                    List.tail input |> Maybe.withDefault []
            in
            if head == "" then
                -- "" separates the records we we are the end of a chunk, add the complete group to all
                -- groups and start a new blank group
                let
                    -- without this check it is possible to have an empty group at the end of the group list
                    allGroups_ =
                        if List.isEmpty group then
                            ...
                            group :: allGroups
                in
                chunk [] allGroups_ tail

            else
                -- group not finished
                chunk (head :: group) allGroups tail
